...
| Research claim guard                | `migrations/20260806124500_claim_skeleton_for_research.sql`                | `replace_current_food_source` gains `p_require_unclaimed`; under that flag it re-checks skeleton state inside its own row lock and returns `claim_status = 'conflict'` without writing. |
...
- Do not make automated output appear human-verified or published by setting `data_verified_at`, `published_at`, `published_by`, or `verification_method`. Two paths may set them, both requiring a human: `publish_food_draft` (DRAFT → published, evidence-checked) and the direct `/api/foods` insert when `authorization.origin === "human"` — hand-entered label data has no source transcript to check against, so the curator is the verification. An automation credential on `/api/foods` leaves the row a draft. See `docs/plans/2026-08-06-unify-publication-paths.md` for the open question of whether the direct path should keep that exemption.
