...
BEGIN
  ...
  LOOP
    ...
    ELSIF v_existing_source_kind IS DISTINCT FROM v_source_kind THEN
      v_status := 'skipped';
    ELSIF v_existing_value IS DISTINCT FROM v_value THEN
      v_status := 'conflict';
    ELSE
      UPDATE public.foods
      SET nutrient_sources = coalesce(nutrient_sources, '{}'::jsonb)
          || jsonb_build_object(v_nutrient_key, v_source_kind::text),
          updated_at = statement_timestamp()
      WHERE id = p_food_id;
      v_status := 'applied';
    END IF;
