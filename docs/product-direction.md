...
2. **Fix the shape before re-extraction freezes it.** The minimum model is `{name, position, form}`. `position` carries the descending-weight order that labels encode and is the actual value of ingredient data; `form` preserves the raw-material distinction (닭고기 / 계육분 / 가금육) that the current `meat | fish | plant | other` typing discards. `pct` is not modelled because these labels rarely state it. Eight source files consume the field today, excluding tests and generated types.
