...
WITH ranked AS (
  SELECT f.id, f.brand_id, f.product_name,
         row_number() OVER (
           PARTITION BY f.brand_id, f.product_name
...
survivors AS (
  SELECT brand_id, product_name, id AS survivor_id FROM ranked WHERE rn = 1
)
SELECT r.id AS loser_id, s.survivor_id
FROM ranked r
JOIN survivors s ON s.brand_id = r.brand_id AND s.product_name = r.product_name
WHERE r.rn > 1;
...
BEGIN
  SELECT count(*) INTO dup FROM (
    SELECT brand_id, product_name FROM foods GROUP BY 1,2 HAVING count(*) > 1
  ) x;
