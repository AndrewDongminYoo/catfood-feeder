...
| Draft rows remain readable through the public Supabase policy                            | The app filters `data_verified_at IS NULL`, but the base `foods_select` RLS policy allows direct anonymous reads. Restricting drafts requires a new policy migration and confirmation that direct draft access is not intentional.                                                                                                               |
