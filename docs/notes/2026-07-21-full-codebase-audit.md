...
The fix pass did not touch `src/types/supabase.d.ts` (a generated artifact) or the curator workspace UI.
...
| `src/types/supabase.d.ts` drift (`extraction_rate_limits` missing, `food_sources.kind` too wide) | Generated artifact. Run the Supabase CLI type generation after `supabase db push`; hand-editing it is against the repo's own rule.                                                                                                                 |
...
- `extraction_rate_limits` (migration 0006) is absent from `src/types/supabase.d.ts` — 9 tables in migrations, 8 in types. Nothing breaks today only because `request-rate-limit.ts:19` calls the RPC over raw REST.
