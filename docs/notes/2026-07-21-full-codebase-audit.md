...
- Source refresh now returns an explicit result for every candidate.
  An equal same-kind refresh repoints current evidence to the new capture without touching the food row, a changed same-kind refresh returns `conflict` while retaining the historical evidence, and a different-kind overlap returns `skipped` without changing the stored value or provenance (`20260721121348_source_refresh_provenance.sql`, corrected by `20260721131159_compare_evidence_precision.sql` and `20260721133137_evidence_only_equal_refresh.sql`).
...
- `pnpm test` exists and collects `src/**/*.test.{ts,tsx}`; the current suite went from collecting 191 vendor failures to 8 passing TypeScript files / 63 tests.
...
- Successful source replacement now compares the incoming normalized `content_hash` with the current same-food, same-kind capture and returns `initial`, `unchanged`, or `changed`. The curator UI warns only when the transcript changed; capture still does not mutate nutrient values or evidence (`20260723061841_content_hash_change_detection.sql`).
- `consume_extract_quota` is now service-role-only: `EXECUTE` was revoked from `PUBLIC`, `anon`, and `authenticated` while preserved for `service_role`, so the database-backed extraction quota RPC cannot be invoked by public API roles (`20260722015424_restrict_extract_quota_execute.sql`). `supabase/tests/extraction_quota_privileges_test.sql` asserts the ACL and exercises the behavior: `anon`/`authenticated` calls raise `42501`, `service_role` consumes quota slots.
...
- **`content_hash` was written but never compared** (fixed). The source-replacement transaction now compares the current same-kind capture with the incoming hash and reports whether the transcript is initial, unchanged, or changed without touching nutrients or evidence.
...
- The plan now has 33 checked steps and 4 incomplete or partial steps with explicit reasons.
