...
| Source refresh can leave stale current evidence                                          | Applying a refreshed source skips populated nutrients, so their evidence can still point to the retired source. The safe behavior for equal versus conflicting refreshed values needs an explicit RPC outcome and curator policy.                                                                                                                |
