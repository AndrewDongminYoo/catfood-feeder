    ...
    try {
      ...
      const transcript = await runCodex(
        [
          ...
          "a customer review.",
          "",
          "Each excerpt must contain EXACTLY ONE number: the value you are reporting.",
          "The server rejects an excerpt holding a second number, and it rejects the whole",
          "batch when one entry fails — so a citation quoted too wide throws away every",
          "other value with it. Quote the shortest span that still carries the number:",
          "  good: '조단백 (이상)  Crude Protein (min.)  35.0 %'",
          "  good: '3,870 kcal/kg'",
          "  bad:  '대사에너지(ME)  3,870 kcal/kg(NRC 2006)'   ← 3,870 and 2006",
          "Standards and years printed beside a value (NRC 2006, AAFCO 2016) are the usual",
          "way this goes wrong. Leave them outside the excerpt.",
          "Return only the JSON object described by the output schema.",
        ].join("\n"),
        TRANSCRIPT_SCHEMA,
        workdir,
