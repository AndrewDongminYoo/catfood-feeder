  ...
  try {
    discovery ??= await runCodex(
      [
        ...
        "",
        "How to find the page — do this, do not guess a URL:",
        "1. Fetch the brand site above and extract every product href from the HTML.",
        "   These sites use their own URL shape (this one is /shop_view/?idx=NNN), and a",
        "   URL assembled from a search result is usually a 404.",
        "2. Open the candidates and match on the page title, not on the URL.",
        "3. Report a productPageUrl ONLY after you have fetched it and seen the product",
        "   name on the page. If you did not fetch it, it does not count as found.",
        "",
        "Many brands list only their current lineup, so a product in the list above may",
        "simply not exist on the site. That is a normal outcome, not a reason to keep",
        "searching or to fall back on a marketplace listing.",
        "",
        "Rules:",
        ...
        "  A URL you did not open is not a find — return null instead.",
    ...
    try {
      ...
      const transcript = await runCodex(
        [
          ...
          "a customer review.",
          "",
          "Every excerpt must satisfy BOTH of these, or it is useless:",
          "",
          "1. EXACTLY ONE number — the value you are reporting. The server rejects an",
          "   excerpt holding a second number, and it rejects the whole batch when one",
          "   entry fails, so a citation quoted too wide throws away every other value.",
          "   A standard or year printed beside the figure (NRC 2006, AAFCO 2016) is the",
          "   usual way this happens. Leave it outside the excerpt.",
          "2. The nutrient's OWN LABEL, as printed. An excerpt is what proves this number",
          "   belongs to this nutrient; without the label it proves only that some number",
          "   appears. Two excerpts must never be identical — 조회분 and 수분 are often",
          "   both '12.0% 이하', and quoting them that way makes both citations worthless.",
          "",
          "  good: '조회분 12.0% 이하'",
          "  good: '조단백 (이상)  Crude Protein (min.)  35.0 %'",
          "  bad:  '12.0% 이하'                        ← which nutrient?",
          "  bad:  '대사에너지(ME)  3,870 kcal/kg(NRC 2006)'   ← 3,870 and 2006",
          "Return only the JSON object described by the output schema.",
        ].join("\n"),
        TRANSCRIPT_SCHEMA,
        workdir,
